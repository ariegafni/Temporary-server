...
export class LocationsManager {
  static async getCountriesWithHosts() {
    ...
    for (const h of hosts) {
      if (!map.has(h.country_place_id)) {
        map.set(h.country_place_id, []);
      }
      map.get(h.country_place_id)!.push(h);
    }

    return Array.from(map.entries()).map(([country_place_id, hosts]) => ({
      country_place_id,
      hosts,
    }));
  }
  ...
  static async getCitiesByCountry(country_place_id: string) {
    ...
    const mapCounts = new Map<string, number>(
      agg.map((a: any) => [a._id, a.host_count])
    );
    const cities = await CityModel.find({
      place_id: { $in: Array.from(mapCounts.keys()) },
      country_place_id,
    }).lean();
    const merged = cities.map((c) => ({
      ...c,
      host_count: mapCounts.get(c.place_id) || 0,
    }));
    const missingIds = Array.from(mapCounts.keys()).filter(
      (pid) => !cities.find((c) => c.place_id === pid)
    );
    const missing = missingIds.map((pid) => ({
      place_id: pid,
      name: pid,
      country_place_id,
      host_count: mapCounts.get(pid) || 0,
    }));
    return [...merged, ...missing].sort(
      (a, b) => (b.host_count || 0) - (a.host_count || 0)
    );
  }
  ...
  static async search(params: {
    ...
    const countries = await CountryModel.find(
      params.query ? { $text: { $search: params.query } } : {}
    )
      .limit(limit)
      .lean();
    ...
    if (params.country_place_id)
      citiesQuery.country_place_id = params.country_place_id;
  ...
  static async popular() {
    const countries = await this.getCountriesWithHosts();
    ...
    const citiesAgg = await HostModel.aggregate([
      {
        $group: {
          _id: {
            ...
            country_place_id: "$country_place_id",
          },
          host_count: { $sum: 1 },
        },
      },
